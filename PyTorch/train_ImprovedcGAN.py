"""
 > Training pipeline for ImprovedcGAN (paired) model
"""
# py libs
import os
import sys
...
from nets.ImprovedcGAN import GeneratorImprovedcGAN, DiscriminatorImprovedcGAN
...
samples_dir = os.path.join("samples/ImprovedcGAN/", dataset_name)
checkpoint_dir = os.path.join("checkpoints/ImprovedcGAN/", dataset_name)
os.makedirs(samples_dir, exist_ok=True)
os.makedirs(checkpoint_dir, exist_ok=True)


""" ImprovedcGAN specifics: loss functions and patch-size
-----------------------------------------------------"""
...
generator = GeneratorImprovedcGAN()
discriminator = DiscriminatorImprovedcGAN()
...
if args.epoch == 0:
    ...
    generator.load_state_dict(torch.load("checkpoints/ImprovedcGAN/%s/generator_%d.pth" % (dataset_name, args.epoch)))
    discriminator.load_state_dict(torch.load("checkpoints/ImprovedcGAN/%s/discriminator_%d.pth" % (dataset_name, epoch)))
...
for epoch in range(epoch, num_epochs):
    for i, batch in enumerate(dataloader):
        ...
        if batches_done % val_interval == 0:
            ...
            save_image(img_sample, "samples/ImprovedcGAN/%s/%s.png" % (dataset_name, batches_done), nrow=5, normalize=True)

    ## Save model checkpoints
    if (epoch % ckpt_interval == 0):
        torch.save(generator.state_dict(), "checkpoints/ImprovedcGAN/%s/generator_%d.pth" % (dataset_name, epoch))
        torch.save(discriminator.state_dict(), "checkpoints/ImprovedcGAN/%s/discriminator_%d.pth" % (dataset_name, epoch))
